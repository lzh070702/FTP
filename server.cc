...
void set_nonblock(int fd);        // 设置非阻塞
void cmd_exec(int epfd, int fd);  // 执行命令
    ...
    while (true) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (fd == ctrl_lfd) {
                ...
                pool.enqueue([epfd, fd]() { cmd_exec(epfd, fd); });
            }
        }
    }
    close(ctrl_lfd);
...
void cmd_exec(int epfd, int fd) {
    ...
    if (cmd_code == 1) {
        ...
        if (chdir(target_dir.c_str()) != 0) {
            ...
            strcpy(response.text, "Directory successfully changed.");
        }
        ...
        if (cmd_code == 4) {
            ...
            strcpy(response.text, "Here comes the directory listing.");
            ...
            if (out_fd < 0) {
                response.stus_code = 550;
                strcpy(response.text, "Failed to open file for writing.");
                send(fd, (char*)&response, sizeof(response), 0);
                return;
            }
            response.stus_code = 150;
            strcpy(response.text, "Opening BINARY mode data connection for");
            response.file_size = lseek(out_fd, 0, SEEK_END);
            lseek(out_fd, 0, SEEK_SET);
            send(fd, (char*)&response, sizeof(response), 0);
            off_t offset = 0;
            while (offset < response.file_size) {
                ssize_t sent = sendfile(data_cfd, out_fd, &offset,
